...
class isYoutubeLink(asyncio_filters.SimpleCustomFilter):
	...
	async def check(message: types.Message):
		search = re.search(r'^((?:https?:)?\/\/)?((?:www|m)\.)?((?:youtube(-nocookie)?\.com|youtu.be))(\/(?:[\w\-]+\?v=|embed\/|v\/)?)([\w\-]+)(\S+)?$', str(message.text))
		return bool(search)

@bot.message_handler(commands=['help', 'start'])
async def send_message(message):
	msg = await bot.send_message(message.chat.id, "Hola, soy un bot sin un objetivo, pero que puede hacer muchas cosas, por ahora puedo hacer lo siguiente:",reply_markup=genMarkup())
...
async def getAudioFromVideo(call):
    try:
        ...
        with mock.patch('pytube.cipher.get_throttling_plan', patched_throttling_plan):
            ...
            out_file = "file"
            # VALIDATE PATH FOR AUDIOS
            if os.path.exists(os.path.join(os.getcwd(), "audios")):
                out_file = audio.download(os.path.join(os.getcwd(), "audios")) #type: ignore
            else:
